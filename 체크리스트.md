...
*   **2.2 데이터베이스 (Cloudflare D1)**
    ...
    *   [ ] 캐싱 전략 (`unstable_cache`, `revalidateTag`)이 D1 쿼리에 적용되어 있는가? (미구현)

*   **2.3 스토리지 (Cloudflare R2)**
    *   [ ] `R2_ASSETS` 환경 변수 바인딩을 통해 R2 버킷에 접근하고 있는가? (바인딩은 설정되었으나 코드에서 미사용)
    *   [ ] R2를 이미지 호스팅에 사용하고 있으며, 퍼블릭 읽기(Custom Domain) 및 인증된 쓰기(Presigned URL)가 구현되었는가? (미구현)
    *   [ ] `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` 환경 변수가 올바르게 설정되어 있는가?

*   **2.4 인증 (NextAuth.js)**
    *   [ ] NextAuth.js 기반의 인증 시스템이 구현되었는가? (미구현, 현재 모의(mock) 인증 사용 중)
    *   [ ] D1을 백엔드로 사용하는 커스텀 어댑터가 구현되어 있는가? (미구현, NextAuth.js 미도입)

*   **2.5 개발/배포 환경**
    *   [x] GitHub (Tenant Repos & CMS Repo)를 소스 코드로 사용하고 있는가? (확인 완료: `PROJECT_CONTEXT.md` 및 서버 액션에서 GitHub 사용이 명시 및 구현됨.)
    *   [x] Cloudflare Pages Build System을 CI/CD에 활용하고 있는가? (확인 완료: `PROJECT_CONTEXT.md` 및 `wrangler.toml`을 통해 확인됨.)

## 3. 핵심 기능 구현 현황 (Core Feature Implementation Status)

*   **3.1 사이트 프로비저닝 워크플로우**
    *   [ ] 슈퍼 어드민이 신규 사이트를 생성하는 UI 및 백엔드 로직이 구현되었는가? (미구현: UI 없음, 백엔드 로직은 플레이스홀더 상태)
    *   [ ] GitHub API를 사용하여 템플릿 레포지토리를 복제하고 `site-config.json`을 수정하는 기능이 구현되었는가? (미구현)
    *   [ ] Cloudflare API를 호출하여 커스텀 호스트명 등록 및 TXT 레코드를 발급하는 기능이 구현되었는가? (미구현)
    *   [ ] D1 `tenants` 테이블에 레코드를 삽입하고, 초기 `status`를 `PENDING_DNS`로 설정하는 로직이 구현되었는가? (미구현)

*   **3.2 CMS 에디터 및 이미지 파이프라인**
    *   [ ] CMS 에디터에서 이미지 드래그 앤 드롭 기능이 구현되었는가? (미구현)
    *   [ ] 클라이언트가 `/api/upload/presigned` 엔드포인트를 통해 R2 업로드용 Presigned URL을 요청하는 기능이 구현되었는가? (미구현)
    *   [ ] 서버(Worker)에서 `tenant_id`를 검증하고 R2 `PutObjectCommand`를 사용하여 Presigned URL을 생성하는 로직이 구현되었는가? (미구현)
    *   [ ] 클라이언트가 발급받은 Presigned URL로 직접 R2에 이미지를 업로드하는 기능이 구현되었는가? (미구현)
    *   [ ] 이미지 업로드 성공 시 마크다운 본문에 이미지 URL(`![alt](https://assets.endpr.io/.../image.webp)`)이 올바르게 삽입되는가? (미구현)

*   **3.3 발행 시스템 (Publishing Engine)**
    *   [ ] 'Publish' 버튼 클릭 시 필수 필드(제목, 슬러그 등) 검증 로직이 동작하는가? (미구현)
    *   [x] D1 `posts` 테이블의 `status`가 `PUBLISHING` -> `PUBLISHED`로 올바르게 변경되는가? (확인 완료: 성공 및 실패 시 상태 변경/롤백 로직 구현됨)
*   [x] Frontmatter (JSON)와 본문 (Markdown)을 결합하여 `--- 
 {json} 
 --- 
 {markdown}` 형식의 마크다운 파일을 생성하는 기능이 구현되었는가? (확인 완료: `post.ts`에서 명시적으로 구현됨)
    *   [x] GitHub API (`createOrUpdateFileContents`)를 호출하여 `content/posts/{slug}.mdx` 경로에 파일을 커밋하는 기능이 구현되었는가? (확인 완료: `create_or_update_file` 함수를 통해 구현됨)
    *   [x] Cloudflare Pages의 Deploy Hook을 트리거하거나 자동 감지되도록 설정되어 있는가? (확인 완료: GitHub 커밋 시 자동 감지되도록 설계됨)
    *   [x] 발행 완료 후 D1 `deployments` 테이블에 배포 로그가 정확히 기록되는가? (확인 완료: 단, 커밋 SHA는 임시 값)

## 4. 엣지 케이스 및 에러 핸들링 (Edge Cases & Error Handling)

*   **4.1 GitHub API Rate Limit 처리**
    *   [ ] 동시에 대량의 발행 요청 발생 시 Cloudflare Queues를 도입하여 요청을 큐에 적재하고 스로틀링하는 메커니즘이 설계 또는 구현되었는가? (미구현, 설계만 반영됨)

*   **4.2 도메인 연결 지연 처리**
    *   [ ] 고객 도메인 연결 지연(DNS 설정 미비 등) 시 CMS 대시보드에 "도메인 연결 대기 중" 배너가 표시되는가? (미구현)
    *   [ ] 주기적으로(Cron Trigger) Cloudflare API를 통해 도메인 상태를 확인하고 D1 `tenants` 테이블을 업데이트하는 로직이 구현되었는가? (미구현)

*   **4.3 데이터 충돌 (동시성) 처리**
    *   [x] 사용자 A와 B가 동시에 같은 글을 수정할 경우를 대비하여 `posts` 테이블에 `version` 컬럼을 이용한 낙관적 락(Optimistic Locking)이 적용되었는가? (확인 완료: `d1.ts`의 `updatePost`에서 `version`을 사용함)
    *   [x] 충돌 발생 시 사용자에게 "다른 사용자가 수정 중입니다"와 같은 명확한 알림이 제공되는가? (확인 완료: 백엔드에서 명확한 에러 메시지를 반환함)

## 5. 개발 가이드라인 준수 및 환경 설정 (Development Guidelines & Environment Setup)

*   **5.1 엄격한 타입 지정 (Strict Typing)**
    *   [x] 모든 데이터 입출력(API 요청/응답, DB 스키마)에 Zod 스키마를 사용하여 유효성 검사 및 타입 안전성을 확보하고 있는가? (확인 완료: `d1.ts`에서 입출력 데이터에 `schema.parse`를 적용함)

*   **5.2 배포 환경 설정**
    *   [x] Cloudflare Pages 배포 시 `DB`, `R2_ASSETS` 바인딩이 올바르게 설정되었는가? (확인 완료: `wrangler.toml`에 정의됨)
    *   [ ] `GITHUB_TOKEN`, `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` 환경 변수(비밀 값)가 안전하게 설정되었는가? (확인 필요: 외부 환경 설정)
    *   [ ] `d1/schema.sql`을 이용한 D1 스키마 마이그레이션이 배포 후 정상적으로 이루어지는지 확인되었는가? (수동 설정 필요: README.md에 가이드됨)
    *   [x] 초기 데이터 시딩(테넌트, 슈퍼 어드민 사용자) 가이드 또는 스크립트가 명확하게 제공되는가? (확인 완료: README.md에 가이드 제공됨)

*   **5.3 로컬 개발 환경**
    *   [x] `npm install` 및 `npx wrangler d1 execute ... --local` 명령을 통해 로컬 D1 설정이 원활하게 이루어지는가? (확인 완료: README.md에 가이드 제공됨)
    *   [x] `npx wrangler dev --local --experimental-local-pages` 명령과 필요한 환경 변수 설정을 통해 로컬 개발 서버가 정상적으로 동작하는가? (확인 완료: README.md에 가이드 제공됨)
